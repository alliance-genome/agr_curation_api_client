#!/usr/bin/env python3
"""Test FBDVTerm ontology search using DatabaseMethods.

FBDVTerm - FlyBase Developmental Ontology
Record count: 210 records
Test terms: "larval", "adult", "embryo"
"""
...
class TestFBDVTerm(unittest.TestCase):
    """Test FBDVTerm searches using unified ontologyterm table."""

    @classmethod
    def setUpClass(cls):
        cls.db = DatabaseMethods()
        cls.ontology_type = 'FBDVTerm'
    ...
    def test_prefix_match_adult(self):
        """Test prefix match search for 'adult'."""
        results = self.db.search_ontology_terms(
            term='adult',
            ontology_type=self.ontology_type,
            limit=10
    ...
    def test_contains_match_embryo(self):
        """Test contains match search for 'embryo'."""
        results = self.db.search_ontology_terms(
            term='embryo',
            ontology_type=self.ontology_type,
            limit=10
        ...
        self.assertIsInstance(results, list)
        self.assertGreater(len(results), 0, "Should find embryo-related results")
        for result in results:
            self.assertEqual(result.ontology_type, self.ontology_type)
