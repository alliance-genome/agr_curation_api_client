...
class TestTieredSearchIntegration:
    ...
    def test_gene_synonym_search(self, db_methods):
        ...
        assert any('rutabaga' in r['entity'].lower() for r in results), \
            "Expected to find 'rutabaga' in results"
    ...
    def test_gene_without_synonyms(self, db_methods):
        ...
        assert len(with_synonyms) >= len(without_synonyms), \
            "Expected more results when synonyms included"
    ...
    def test_relevance_ordering(self, db_methods):
        ...
        assert relevance_scores == sorted(relevance_scores), \
            "Results should be ordered by relevance (1=exact, 2=starts_with, 3=contains)"

        # Verify relevance scores are valid
        assert all(score in [1, 2, 3] for score in relevance_scores), \
            "All relevance scores should be 1, 2, or 3"
