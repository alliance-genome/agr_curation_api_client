#!/usr/bin/env python3
"""Test ZECOTerm ontology search using DatabaseMethods.

ZECOTerm - Zebrafish Experimental Conditions Ontology
Record count: 161 records
Test terms: "chemical", "temperature", "genetic"
"""
...
class TestZECOTerm(unittest.TestCase):
    """Test ZECOTerm searches using unified ontologyterm table."""

    @classmethod
    def setUpClass(cls):
        cls.db = DatabaseMethods()
        cls.ontology_type = 'ZECOTerm'
    ...
    def test_contains_match_genetic(self):
        """Test contains match search for 'genetic'."""
        results = self.db.search_ontology_terms(
            term='genetic',
            ontology_type=self.ontology_type,
            limit=10
        ...
        self.assertIsInstance(results, list)
        # May not find results for 'genetic' in ZECO ontology, just verify structure
        for result in results:
            self.assertEqual(result.ontology_type, self.ontology_type)
