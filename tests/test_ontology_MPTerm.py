#!/usr/bin/env python3
"""Test MPTerm ontology search using DatabaseMethods.

MPTerm - Mammalian Phenotype Ontology
Record count: 14,451 records
Test terms: "abnormal", "lethality", "behavior"
"""
...
class TestMPTerm(unittest.TestCase):
    """Test MPTerm searches using unified ontologyterm table."""

    @classmethod
    def setUpClass(cls):
        cls.db = DatabaseMethods()
        cls.ontology_type = 'MPTerm'
    ...
    def test_prefix_match_lethality(self):
        """Test prefix match search for 'lethality'."""
        results = self.db.search_ontology_terms(
            term='lethality',
            ontology_type=self.ontology_type,
            limit=10
        ...
        self.assertIsInstance(results, list)
        self.assertGreater(len(results), 0, "Should find lethality-related results")
        for result in results:
            self.assertEqual(result.ontology_type, self.ontology_type)

    def test_contains_match_behavior(self):
        """Test contains match search for 'behavior'."""
        results = self.db.search_ontology_terms(
            term='behavior',
            ontology_type=self.ontology_type,
            limit=10
        ...
        self.assertIsInstance(results, list)
        self.assertGreater(len(results), 0, "Should find behavior-related results")
        for result in results:
            self.assertEqual(result.ontology_type, self.ontology_type)
