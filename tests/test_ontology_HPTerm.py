#!/usr/bin/env python3
"""Test HPTerm ontology search using DatabaseMethods.

HPTerm - Human Phenotype Ontology
Record count: 19,232 records
Test terms: "abnormal", "seizure", "growth"
"""
...
class TestHPTerm(unittest.TestCase):
    ...
    def test_prefix_match_seizure(self):
        """Test prefix match search for 'seizure'."""
        results = self.db.search_ontology_terms(
            term='seizure',
            ontology_type=self.ontology_type,
            limit=10
        ...
        self.assertIsInstance(results, list)
        self.assertGreater(len(results), 0, "Should find seizure-related results")
        for result in results:
            self.assertEqual(result.ontology_type, self.ontology_type)

    def test_contains_match_growth(self):
        """Test contains match search for 'growth'."""
        results = self.db.search_ontology_terms(
            term='growth',
            ontology_type=self.ontology_type,
            limit=10
        ...
        self.assertIsInstance(results, list)
        self.assertGreater(len(results), 0, "Should find growth-related results")
        for result in results:
            self.assertEqual(result.ontology_type, self.ontology_type)
